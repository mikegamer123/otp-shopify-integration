...
module.exports = {
  ...
  async getOrder(id) {
    return {
      id: Number(id),
      name: `#MOCK${id}`,
      // Shape-matches the real thing: the token in the path is what makes the
      // page viewable by the customer without logging in.
      order_status_url: `https://example.myshopify.com/orders/mock-token-${id}`,
    };
  },
