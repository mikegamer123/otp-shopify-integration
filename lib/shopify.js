...
function isRetryable(method, path, err, res) {
  ...
  if (err) {
    ...
    //
    // EAI_AGAIN is a transient DNS failure and it is in this list because of a
    // real observation, not for completeness: the first request out of a
    // freshly started container failed with it while the resolver was still
    // warming up, and a second attempt a moment later succeeded. That is
    // precisely what a cold-started free-tier instance does on its first
    // request after sleeping, so without this a customer's checkout fails on
    // the exact request that wakes the service up.
    return (
      err.type === "request-timeout" ||
      ["ECONNRESET", "ETIMEDOUT", "ECONNREFUSED", "EPIPE", "ENOTFOUND", "EAI_AGAIN"].includes(err.code)
    );
  }
...
const real = {
    ...
    if (shippingRateHandle) {
      ...
      return {
        ...
        shipping_line: d.shippingLine
          ...
          : null,
        // Normalised to REST's shape so callers don't branch on which API made
        // the draft — the payment page itemises these.
        line_items: (d.lineItems?.nodes || []).map((li) => ({
          title: li.title,
  ...
  async getOrder(id) {
    const { order } = await call("GET", `/orders/${id}.json`);
    return order;
  },
